-- Phase 6B: Server-side rating implementation

-- 1. Extend rooms table
ALTER TABLE public.rooms ADD COLUMN IF NOT EXISTS finalized_at TIMESTAMPTZ;
ALTER TABLE public.rooms ADD COLUMN IF NOT EXISTS game_id UUID;

-- 2. Extend games table
ALTER TABLE public.games ADD COLUMN IF NOT EXISTS source_room_id UUID UNIQUE REFERENCES public.rooms(id) ON DELETE SET NULL;
ALTER TABLE public.rooms ADD CONSTRAINT fk_rooms_game_id FOREIGN KEY (game_id) REFERENCES public.games(id) ON DELETE SET NULL;

-- 3. Create rating_events table
...
CREATE POLICY "Users can view own rating events" ON public.rating_events FOR SELECT USING (auth.uid() = user_id);

-- 4. Create finalize_online_room RPC
...
DECLARE
  ...
  v_game_id uuid;
BEGIN
  -- Block authenticated browser clients completely.
  ...
  -- Ensure both users exist in room
  ...
  IF v_room.game_mode = 'ranked' THEN
    ...
    IF v_room.result = 'white' THEN
      v_w_score := 1.0; v_b_score := 0.0;
    ELSIF v_room.result = 'black' THEN
    ...
    -- Update profiles with rating & counters
  ...
  ELSE
    -- Casual: Counters only
    UPDATE public.profiles
    SET games_played = games_played + 1,
        wins = wins + CASE WHEN v_room.result = 'white' THEN 1 ELSE 0 END,
        losses = losses + CASE WHEN v_room.result = 'black' THEN 1 ELSE 0 END,
        draws = draws + CASE WHEN v_room.result = 'draw' THEN 1 ELSE 0 END
    WHERE user_id = v_room.white_user_id;

    UPDATE public.profiles
    SET games_played = games_played + 1,
        wins = wins + CASE WHEN v_room.result = 'black' THEN 1 ELSE 0 END,
        losses = losses + CASE WHEN v_room.result = 'white' THEN 1 ELSE 0 END,
        draws = draws + CASE WHEN v_room.result = 'draw' THEN 1 ELSE 0 END
    WHERE user_id = v_room.black_user_id;
  END IF;
  ...
  INSERT INTO public.games (
    ...
    v_room.result, COALESCE(v_room.end_reason, 'unknown'), v_room.pgn, v_moves_count,
  ...
  -- Create rating events if ranked
  IF v_room.game_mode = 'ranked' THEN
...
-- End of Phase 6B Migration.
-- Note: Profile stat mutation lockdown deferred to Phase 6C.
