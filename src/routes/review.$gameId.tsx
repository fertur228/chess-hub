...
export const Route = createFileRoute("/review/$gameId")({
  ...
  component: () => <RequireAuth><AppShell><Review /></AppShell></RequireAuth>,
});
...
function Review() {
  ...
  useEffect(() => {
    supabase.from("games").select("*").eq("id", gameId).maybeSingle().then(({ data }) => {
      setG(data); setLoading(false);
    });
  }, [gameId]);

  if (loading) return <PageContainer><Skeleton className="h-96 w-full" /></PageContainer>;
  if (!g) return <PageContainer><div className="text-center text-muted-foreground py-12">Game not found.</div></PageContainer>;

  const { opponent, result, delta } = user ? gameForUser(g, user.id) : { opponent: "Opponent", result: "draw" as const, delta: null };
  const moments: { title: string; text: string }[] = g.key_moments || [];
  ...
  return (
    <PageContainer>
      <div className="max-w-3xl mx-auto">
        ...
        <h1 className="font-display text-3xl font-bold">{result === "win" ? "Nice win" : result === "loss" ? "Tough one" : "It's a draw"}</h1>
        ...
        <div className="card-surface p-6 mt-5">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              ...
              <div className="font-display text-xl font-bold mt-0.5">vs {opponent}{g.ai_difficulty ? ` (${g.ai_difficulty})` : ""}</div>
            </div>
            <ResultBadge result={result} />
          </div>
          <div className="grid grid-cols-3 gap-4 mt-4 pt-4 border-t text-sm">
            <div><div className="text-xs text-muted-foreground">Type</div><div className="mt-0.5"><GameTypeBadge type={g.game_type} /></div></div>
            <div><div className="text-xs text-muted-foreground">Moves</div><div className="font-semibold mt-0.5">{g.moves_count}</div></div>
            <div><div className="text-xs text-muted-foreground">Date</div><div className="font-semibold mt-0.5">{new Date(g.created_at).toLocaleDateString()}</div></div>
          </div>
        </div>
          ...
          {g.game_type === "Ranked" && ratingBefore != null ? (
            <>
              <div className="flex items-baseline gap-3 mt-1">
                ...
                <div className={`font-display text-xl font-bold ml-2 ${delta && delta > 0 ? "text-success" : delta && delta < 0 ? "text-destructive" : ""}`}>
                  {delta != null ? `${delta > 0 ? "+" : ""}${delta}` : ""}
                </div>
              </div>
              <p className="text-sm text-muted-foreground mt-2">
                {result === "win" ? "You gained rating because you won this ranked game." : result === "loss" ? "You lost rating, but every game makes you better." : "Rating barely changed — you drew."}
              </p>
            </>
          ) : (
            <p className="font-semibold mt-1">Rating unchanged. {g.game_type === "AI Training" ? "AI training games do not affect rating." : "Casual games do not affect rating."}</p>
          )}
        </div>

        <div className="grid sm:grid-cols-3 gap-3 mt-5">
          {moments.length === 0 ? (
            <div className="col-span-3 card-surface p-5 text-sm text-muted-foreground">No detailed moments for this game.</div>
          ) : moments.map((m, i) => (
        ...
        {g.coach_note && (
          <div className="card-surface p-6 mt-5 bg-gradient-to-br from-primary-soft to-card">
            ...
            <p className="font-medium">{g.coach_note}</p>
          </div>
        )}

        <div className="flex flex-wrap gap-2 mt-8">
          <Button asChild><Link to="/play">Play again</Link></Button>
